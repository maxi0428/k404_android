...
class MainActivity : AppCompatActivity() {
    ...
    // 다른함수에서 접근 하기 용이함.
    // 주의 할것은 기본은
    //만약, lateinit하게 되면, 늦게 초기화 해도 괜찮음.
    lateinit var binding: ActivityMainBinding
    //이름 문자열을 담는 배열
    var datas: MutableList<String>? = null
    //나이 문자열을 담는 배열
    var datas2: MutableList<String>? = null
    // 리사이클러뷰 사용하기 위해서 필요한 재료
    lateinit var adapter: MyAdapter

    //savedInstanceState : Bundle? -> 임시로 번들 객체에 담아주는것.
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // 현재 메인에서 -> 입력 액티비티에서 이름과 나이를 입력하고 그 값을 처리하는 부분이 여기.
        // 입력 액티비티  finish() 호출해서, 메인 액티비티로 넘어 왔고, 인텐트에 담은 데이터를 처리함.
        val requestLauncher = registerForActivityResult(
            ActivityResultContracts.StartActivityForResult())
        {
            it.data!!.getStringExtra("name")?.let {
                //name 의 키에 해당하는 값을 가져온 -> it
                datas?.add(it)
                // 어댑터에 연결된 datas, tatas2 배열값
